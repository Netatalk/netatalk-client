...
with the afpc fs suspend and resume commands.
...
Netatalk Client attempts to detect the mapping type automatically; do 'afpc fs
status' (for FUSE mounts) or 'status' within afpcmd to see what it guessed.
...
guest access. FUSE mounts can use an exact advertised name with `afpc fs
...
It can also be used as a general purpose AFP file server,
which has been tested with Netatalk Client.
It has one notable quirk: it can take up to 20 seconds
for the server to respond when the hard drive is spun down.
For this purpose, Netatalk Client applies a 30 second timeout
for AFP connections when a Time Capsule is detected,
instead of the default 5 second timeout.
