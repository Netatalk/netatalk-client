...
static int ml_getattr_internal(struct afp_volume * volume, const char *path,
                               struct stat *stbuf,
                               struct afp_metadata *metadata)
{
    ...
    if (!metadata) {
        return ll_getattr(volume, converted_path, stbuf, 0);
    }

    ret = ll_getattr_with_metadata(volume, converted_path, stbuf, 0, &fp);

    if (ret == 0) {
        memcpy(metadata->finderinfo, fp.finderinfo,
               sizeof(metadata->finderinfo));
        metadata->resource_size = fp.resourcesize;
    }
...
{
    return ml_getattr_internal(volume, path, stbuf, NULL);
}
                             ...
                             struct afp_metadata *metadata)
{
    ...
    return ml_getattr_internal(volume, path, stbuf, metadata);
}
...
static int ml_getresourcefork_internal(struct afp_volume * volume,
                                       const char *path, void *value,
                                       size_t size, off_t position,
                                       ...
                                       int size_is_known)
{
    ...
    if (position < 0 || (uint64_t)position >= resource_size) {
        return 0;
    }
    ...
    remaining = resource_size - (uint64_t)position;
...
{
    return ml_getresourcefork_internal(volume, path, value, size, position,
                                       0, 0);
}
