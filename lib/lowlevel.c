...
static int ll_getattr_internal(struct afp_volume * volume, const char *path,
                               struct stat *stbuf, int resource,
                               struct afp_file_info *metadata)
{
    ...
    if (volume->server->using_version->av_number < 30) {
        ...
        filebitmap |= kFPRsrcForkLenBit | kFPDataForkLenBit;
    } else {
        filebitmap |= kFPExtRsrcForkLenBit | kFPExtDataForkLenBit;
    }
...
int ll_getattr(struct afp_volume * volume, const char *path, struct stat *stbuf,
               int resource)
{
    return ll_getattr_internal(volume, path, stbuf, resource, NULL);
}
                             ...
                             struct afp_file_info *metadata)
{
    ...
    return ll_getattr_internal(volume, path, stbuf, resource, metadata);
}
